...
type RsvpChoice = "solo" | "couple" | "decline" | null;
  ...
  async function handleRsvpSubmit(e: React.FormEvent) {
    ...
    try {
      await fetch("/api/rsvp", {
        ...
        body: JSON.stringify({ name: name.trim(), rsvp, guests }),
      });
      setSubmitted(true);
    } catch {
      setSubmitted(true);
  ...
  return (
    <div className={`wedding${opened ? " wedding--open" : ""}`}>
          ...
          <div className="calendar">
            <div className="calendar__grid">
              ...
              {CALENDAR_WEEKS.flat().map((day, i) =>
                day === "" ? (
                  <i key={`e-${i}`} className="calendar__empty" />
                ) : (
                  <b
                    key={`d-${i}`}
                    className={
        ...
        <section className="block block--soft">
          ...
          {submitted ? (
            ...
            <form className="rsvp" onSubmit={handleRsvpSubmit}>
              {(
                [
                  { value: "solo", label: "Ооба, жалгыз келем" },
                  { value: "couple", label: "Ооба, бирге келебиз" },
                  { value: "decline", label: "Тилекке каршы, келе албайм" },
                ] as const
              ).map((opt) => (
                <label
                  key={opt.value}
      ...
      {opened && (
        ...
        >
          <span className="music-btn__bars" aria-hidden>
            <i />
            <i />
            <i />
          </span>
          <span className="music-btn__play" aria-hidden />
        </button>
      )}
    </div>
  );
